...
const BugCard = ({ bug, onEdit, onDelete, onclick }) => {
  return (
    <div className="flex items-center bg-black text-yellow-300 border border-yellow-600 rounded p-4 mb-2 overflow-x-auto text-sm whitespace-nowrap">
      ...
      <span onClick={onclick} className="mx-4 min-w-[200px] font-semibold cursor-pointer hover:underline">
        {bug.title}
      </span>
      ...
      <div className="ml-6 flex space-x-2">
        <button
          ...
          className="px-3 py-1 bg-yellow-500 text-black font-semibold rounded hover:bg-yellow-600 transition-colors"
        >
          Edit
        </button>
        <button
          onClick={() => onDelete(bug.id)}
          className="px-3 py-1 border border-yellow-500 text-yellow-400 font-semibold rounded hover:bg-red-600 hover:border-red-600 hover:text-white transition-colors"
        >
          Delete
        </button>
      </div>
